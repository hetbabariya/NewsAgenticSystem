...
def build_coordinator(*, mcp_client: MultiServerMCPClient | None, mcp_tools: list) -> Any:
    ...
    prompt = SystemMessage(
        content=(
            ...
            "Planning discipline:\n"
            "- Think step-by-step: decide which agents are needed and in what order.\n"
            "- You must EVALUATE the output of each sub-agent. If a sub-agent's response is incomplete or suggests a next step, follow through by calling the next appropriate agent.\n"
            "- Do NOT just pass the sub-agent's output to the user if the workflow isn't finished.\n"
            "- For 'ingest_cron', you MUST call collector, then filter, then summarizer sequentially, checking each result.\n"
            "- For 'daily_cron', you MUST call publisher.\n\n"
            "Tool-call discipline:\n"
            "- Prefer 0 tool calls for simple greetings.\n"
            "- You are responsible for the FINAL answer. Summarize what was accomplished across all sub-agents.\n"
            "- If a sub-agent fails, report the error and decide if you can recover or if you must stop.\n\n"
            "Workflow Examples:\n"
            "- News Ingestion: task('collector') -> (success) -> task('filter') -> (success) -> task('summarizer')\n"
            "- Preference Query: task('support', 'What are my preferences?')\n"
            "- User Fact Update: task('memory', 'I am a software engineer')\n\n"
            "Always conclude with a 'Final Answer:' that synthesizes the entire process for the user.\n"
        )
    )
